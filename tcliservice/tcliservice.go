...
type TCLIService interface {
	...
	// Parameters:
	//  - Req
	GetLog(req *TGetLogReq) (r *TGetLogResp, err error)
}
...
// Parameters:
//  - Req
func (p *TCLIServiceClient) GetLog(req *TGetLogReq) (r *TGetLogResp, err error) {
	if err = p.sendGetLog(req); err != nil {
		return
	}
	return p.recvGetLog()
}

func (p *TCLIServiceClient) sendGetLog(req *TGetLogReq) (err error) {
	oprot := p.OutputProtocol
	if oprot == nil {
		oprot = p.ProtocolFactory.GetProtocol(p.Transport)
		p.OutputProtocol = oprot
	...
	p.SeqId++
	if err = oprot.WriteMessageBegin("GetLog", thrift.CALL, p.SeqId); err != nil {
		return
	}
...
func (p *TCLIServiceClient) recvGetLog() (value *TGetLogResp, err error) {
	iprot := p.InputProtocol
	if iprot == nil {
		iprot = p.ProtocolFactory.GetProtocol(p.Transport)
		p.InputProtocol = iprot
	...
	if method != "GetLog" {
		err = thrift.NewTApplicationException(thrift.WRONG_METHOD_NAME, "GetLog failed: wrong method name")
		return
	}
	if p.SeqId != seqId {
		err = thrift.NewTApplicationException(thrift.BAD_SEQUENCE_ID, "GetLog failed: out of sequence response")
		return
	}
	if mTypeId == thrift.EXCEPTION {
	...
	if mTypeId != thrift.REPLY {
		err = thrift.NewTApplicationException(thrift.INVALID_MESSAGE_TYPE_EXCEPTION, "GetLog failed: invalid message type")
		return
	}
...
func (p *tCLIServiceProcessorGetLog) Process(seqId int32, iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
	args := TCLIServiceGetLogArgs{}
	if err = args.Read(iprot); err != nil {
		iprot.ReadMessageEnd()
		x := thrift.NewTApplicationException(thrift.PROTOCOL_ERROR, err.Error())
	...
	var err2 error
	if retval, err2 = p.handler.GetLog(args.Req); err2 != nil {
...
type TCLIServiceGetLogArgs struct {
	Req *TGetLogReq `thrift:"req,1" json:"req"`
}
...
type TCLIServiceGetLogResult struct {
	Success *TGetLogResp `thrift:"success,0" json:"success,omitempty"`
}
